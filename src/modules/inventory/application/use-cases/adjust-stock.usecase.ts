...
export  class AdjustStockUseCase {
    constructor(
     @Inject(IProductVariantRepository)  private readonly productVariantRepository : IProductVariantRepository,
     @Inject(IStockMovementRepository) private  readonly movementRepo: IStockMovementRepository,
    ) {
    }
    ...
    async execute(dto: AdjustStockDto, userId: string, merchantId: string) {
        ...
        const variantExist = await this.productVariantRepository.findById(variantId)
